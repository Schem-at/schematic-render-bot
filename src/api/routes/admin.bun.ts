...
export function setupAdminRoutes(router: Router): void {
	 ...
	 */
	router.get("/api/admin/schemas", async (req) => {
		try {
			const query = getQuery(req);
			const limit = parseInt(query.get("limit") || "100");
			const offset = parseInt(query.get("offset") || "0");
			const sortByRaw = query.get("sortBy") || "created_at";
			const sortOrderRaw = query.get("sortOrder") || "DESC";
			...
			return json({ error: error.message || "Failed to fetch schemas" }, 500);
		}
	});
	 ...
	 */
	router.get("/api/admin/schemas/:hash", async (req) => {
		try {
			const params = getParams(router, "/api/admin/schemas/:hash", req);
